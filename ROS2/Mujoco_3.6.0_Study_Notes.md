...
# Part II. MJCF (MuJoCo XML Format) 구조
...
## 15. mujoco 모듈 주요 기능 (렌더링 등)

```python
renderer = mujoco.Renderer(model, height=480, width=640)       # 이미지 렌더링 객체 생성 함수
renderer.update_scene(data, camera="name")  # 시점 갱신
rgb = renderer.render()                     # 이미지를 numpy 배열로 받기 [RGB - 0~255]
imageio.imwrite("rgb.png", rgb)             # 이미지 저장
...
model.ngeom         # geom 개수
model.njnt          # joint 개수
model.nactuator     # actuator 개수 (= model.nu)
...
model.jnt_type[joint_id]      # 해당 type의 정수값
...
model.actuator_trnid[actuator_id] # 액추에이터가 어떤 객체와 연결되어 있는지 → [id,_]
```
